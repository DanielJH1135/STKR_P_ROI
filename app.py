...
<style>
    ...
    .active-tooltip {
        ...
        cursor: pointer;
        font-weight: bold;
        border-bottom: 2px dashed #46B98C; /* 민트 그린 */
    }
    .active-tooltip a {
...
if generate_pdf:
    if not patient_name or not clinic_name:
        ...
        try:
            ...
            # 3. PDF 메인 문구 교체
            pdf.multi_cell(0, 6, f'환자분께서 {years}년 동안 사용하실 경우, 하루 평균 비용은 약 {int(daily_roi):,}원입니다. 구강 건강을 위한 장기적인 관리 옵션입니다. <br>
            또한 해당 계산결과는 이해를 돕기위한 계산입니다.')
            pdf.ln(5)
